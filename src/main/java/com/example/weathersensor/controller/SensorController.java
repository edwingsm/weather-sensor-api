...
public class SensorController {
    ...
    @Operation(
            ...
            description = "Creates and registers a new sensor in the system with the provided details"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "201",
            ...
            @ApiResponse(
                    responseCode = "409",
                    description = "Sensor with the same identifier already exists",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ErrorResponse.class)
                    )
    ...
    public ResponseEntity<SensorResponseDto> registerSensor(
            @Parameter(description = "Sensor registration details", required = true)
            @Valid @RequestBody SensorRegistrationDto dto
    ) {
    ...
    @Operation(
            ...
            description = "Retrieves a list of all sensors registered in the system"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
     ...
     * GET /api/sensors/location/{location}
     */
    @GetMapping("/location/{location}")
    @Operation(
            summary = "Get sensors by location",
            description = "Retrieves all sensors that are located in the specified location"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Successfully retrieved sensors for the specified location",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = SensorResponseDto.class)
                    )
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "No sensors found for the specified location",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ErrorResponse.class)
                    )
    ...
    public ResponseEntity<List<SensorResponseDto>> getSensorsByLocation(
            @Parameter(
                    description = "Location to filter sensors by",
                    required = true,
                    example = "warehouse-a"
            )
